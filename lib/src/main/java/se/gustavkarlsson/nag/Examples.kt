...
private fun newTermsOfService() {
	val acceptedTosVersion = Nag.get("accepted_tos")?.appVersion ?: -1
...
private fun rateOurApp() {
	...
	val ratedApp = Nag.get("rated_app")
	when {
...
private fun showRatingScreen() {
	AlertDialog.Builder(context)
		...
		.setPositiveButton("Rate") { _, _ ->
			Nag.set("rated_app", "rated")
			// Open play store
		}
		.setNegativeButton("Never") { _, _ ->
			Nag.set("rated_app", "never")
		}
		.setNeutralButton("Later") { _, _ ->
			Nag.set("rated_app", "later")
		}
		.setOnCancelListener {
			Nag.set("rated_app", "later")
		}
